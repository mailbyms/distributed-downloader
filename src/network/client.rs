...
impl ClientNetwork {
    ...
    pub async fn request_download_via_manager(
        ...
        // Send download request command as protobuf message
        let initial_command = InitialCommand {
            command_type: CommandType::DownloadRequest as i32,
        };
                ...
                if String::from_utf8_lossy(&data) == "go_ahead" {
                    ...
                    if let Ok(message) = ProstMessage::decode(&buffer[..n]) {
                        ...
                        if let Some(message::Payload::FileInfoResponse(file_info)) = message.payload {
                            ...
                            // Send acknowledgment to manager as protobuf message
                            let ack_message = crate::proto::distributed_downloader::Message {
                                payload: Some(message::Payload::FileData(b"ready_for_download".to_vec())),
                            };
